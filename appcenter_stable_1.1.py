...
async function initAudio(){{
    ...
    document.getElementById('broadcastStatus').textContent='正在收听...';
    _player.style.display='block';
    ...
    var d=await r.json();
    _currentTs=d.ts;
    console.log('[Audio-6a] /api/status returned — _currentTs:',_currentTs,'url:',d.url);
...
async def api_status():
    global last_updated, latest_url
    with _lock:
        return {"url": latest_url, "ts": last_updated}
...
if __name__ == "__main__":
    ...
    try:
        ...
        public_url = tunnel_manager.start(7860, subdomain="voicetyper")
